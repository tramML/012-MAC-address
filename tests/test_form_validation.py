...
class TestSuccess:
    ...
    def tracker(self):
        tdict = {
            ...
            "slots": {"mac_failures": 0},
            "latest_message": {"intent": {}, "entities": [], "metadata": {}},
    ...
    async def test_validate_mac_address_form_success(dispatcher, tracker, domain):
        ...
        pdb.set_trace()
        # NOTE: the form validation runner requires a DomainDict object
        evt_actual = await form.run(dispatcher, tracker, domain.as_dict())
        evt_expected = [
            SessionStarted(),
